...
from tensorflow.keras.applications import MobileNetV3Large  # Usamos MobileNetV3+ (Large)
from tensorflow.keras.applications.mobilenet_v3 import preprocess_input  # Importar preprocess_input de MobileNetV3+
...
class HandGestureRecognition:
    def __init__(self):
        ...
        # Configurar MobileNetV3+ (Large)
        self.img_height, self.img_width = 224, 224
        self.create_mobilenetv3_model()
        ...
        self.spheres_state = [False] * 5  # Estado de las esferas: todas apagadas inicialmente
    ...
    def create_mobilenetv3_model(self):
        """Crear modelo personalizado con MobileNetV3+ (Large)"""
    ...
    def change_sphere_color(self, gesture):
        ...
        for i in range(5):
            p.changeVisualShape(self.spheres[i], -1, rgbaColor=self.spheres_color[i] + (1,))  # Añadir transparencia al color
